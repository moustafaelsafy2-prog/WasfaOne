...
// ــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ
// استدعاء Gemini (مصَحَّح) مع تنفيذ الأدوات وتحوّطات كاملة
// ــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ
...
async function callGeminiStep({ model, currentContents }) {
  ...
  const body = {
    ...
    safetySettings: [] // لا نريد حظرًا صلبًا على نص غذائي اعتيادي
  };
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return { ok: false, error: msg, raw: text };
    }
    ...
    // اجمع نداءات الأدوات (إن وُجدت)
    const functionCalls = parts
      .map((p) => p?.functionCall)
      .filter(Boolean)
      .map((fc) => ({
        name: fc.name,
        // ملاحظة مهمة: في واجهة Gemini الحديثة، args كائن JSON جاهز (ليس نصًا)
        args: typeof fc.args === "object" ? fc.args : safeParseJSON(fc.args || "{}")
      }));

    // إن لم توجد أدوات، فأعد النص النهائي
    if (!functionCalls.length) {
      const answer = extractTextParts(candidate) || "";
      return { ok: true, final: true, text: answer, finish, safety: candidate?.safetyRatings || [] };
    }

    // وإلا أعد قائمة النداءات لتنفيذها محليًا
    return { ok: true, final: false, calls: functionCalls, finish, safety: candidate?.safetyRatings || [] };
  } catch (e) {
...
async function callGeminiWithTools({ model, messages, memoryBlob }) {
  ...
  while (loop < 6) {
    ...
    const step = await callGeminiStep({ model, currentContents });
    if (!step.ok) throw new Error(step.error || "gemini_step_failed");
    ...
    // نفّذ كل نداء أداة محليًا ثم أضف functionResponse
    for (const fc of step.calls || []) {
      const name = fc.name;
      const args = typeof fc.args === "object" ? fc.args : {};
      ...
      // أعد ردّ الأداة بالشكل الصحيح وفق وثائق Gemini:
  ...
  throw new Error("tool_loop_maxed_out");
}

// ــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ
// اختيار نموذج مع السقوط الاحتياطي (Fallback) — مُحسّن
// ــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــــ
async function generateWithFallback(payload) {
  ...
  for (const model of MODEL_POOL) {
    try {
      ...
      const text = String(out.lastText || "").trim();
      if (text) {
        return { model, text, toolInvocations: out.toolInvocations || [], finish: out.finishReason };
      }
      errors.push(`empty:${model}`);
    } catch (e) {
...
function isOutOfDomain(txt) {
  ...
  const banned = [
    "دواء","تشخيص","سرطان","مضاد حيوي","جرعه","عمليه","مرض نفسي","صور اشعه",
    "تحاليل تفسير طبي","سياسه","ماليات شخصيه","اختراق","برمجه خبيثه","سلاح"
  ];
  ...
  try {
    ...
    return ok({
      ...
      meta: {
        ...
        tokens_hint: approxTokens((event.body || "").length + responseText.length)
      }
    });
  } catch (e) {
    return {
...
function approxTokens(inChars) {
  return Math.round((inChars + Number.EPSILON) / 4);
}
...
function postProcess(text, locale) {
  ...
  t = t.replace(/\n{3,}/g, "\n\n");
  if (locale === "ar") {
    // مكان لأي تحويلات مستقبلية خاصة بالعربية
  }
  return t;
}
